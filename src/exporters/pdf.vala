namespace LFinance {
	...
	internal class PDFModelExporter {
		...
		void generate_expense_diagram(StringBuilder builder,
			...
			builder.append (
				"\\begin{axis}[x tick label style={rotate=45,anchor=east},legend style={at={(0.5,1.1)},anchor=south},width=\\textwidth,height=\\axisdefaultheight, date coordinates in=x, xticklabel=\\month-\\day,");
			builder.append ("xmin=").append (this.latexify_date (r.start_date)).append (",xmax=").append (this.latexify_date (
															      r
															      .
															      end_date));
		...
		void generate_pie_chart_for_months(StringBuilder builder,
			...
			var iter =
				data.values.order_by ((a, b) => a.index == b.index ? 0 : (a.index > b.index ? 1 : -1));
			iter.next ();
			while(true) {
				var month = iter.get ();
		...
		string build_extra_info(Expense expense) {
			...
			for(var i = 0; i < expense._tags.size; i++) {
				...
				ret.append ("\\textcolor[RGB]{%u, %u, %u}{\\textbf{[".printf (tag._rgba[0],
											      tag._rgba[1],
											      tag._rgba[2]));
		...
		void search_for_latex() throws Error {
			...
			throw new PDFModelExporterErrors.COMMANDS_NOT_FOUND (_(
										     "Please install TexLive, LuaLaTeX or XeLaTeX"));
		}
		...
		int compile_document(File file) throws Error {
			...
			var args = this.working ==
				   "latexmk" ? new string[] {this.working, "-pdf", file.get_path (),
							     "-interaction=nonstopmode"} : new string[] {this.working,
													 file.get_path (),
		...
		void compile_document_multiple_times(File file) throws Error {
			...
			for(var i = 1; i <= n; i++) {
				this.progress_update (_("Round %u of %u…").printf (i, n),
