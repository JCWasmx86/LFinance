namespace LFinance {
	internal class BigList : ScrollBox {
		...
		internal void rebuild(TriggerType? type,
			...
			if(type == null) {
				this.accounts = new Expander (_("Accounts"), new AccountActionHandler (func,
												       this.model), "text",
							      false);
				...
				this.locations = new Expander (_("Locations"), new LocationActionHandler (
				...
				foreach(var tag in this.model._tags) {
					...
					this.tags.append_string (
						"<b><span foreground=\"#%02x%02x%02x%02x\" >%s</span></b>".printf (
							colors[0], colors[1], colors[2], colors[3], tag._name));
				}
