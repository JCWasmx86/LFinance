namespace LFinance {
	internal class V1Test {
		...
		static void expense_amount_invalid_type() {
			var f = TestUtils.write_file("{\"version\" : 1, \"data\": [{\"purpose\": \"foo\", \"amount\": []}]}");
			var passed = false;
			try {
				var builder = ModelBuilderFactory.from_file(f.get_path());
				assert(builder is ModelV1Builder);
		...
		static void expense_no_date() {
			var f = TestUtils.write_file("{\"version\" : 1, \"data\": [{\"purpose\": \"foo\", \"amount\": 0}]}");
			var passed = false;
			try {
				var builder = ModelBuilderFactory.from_file(f.get_path());
				assert(builder is ModelV1Builder);
		...
		static void expense_date_invalid_type() {
			var f = TestUtils.write_file("{\"version\" : 1, \"data\": [{\"purpose\": \"foo\", \"amount\": 0, \"date\": 0}]}");
			var passed = false;
			try {
				var builder = ModelBuilderFactory.from_file(f.get_path());
				assert(builder is ModelV1Builder);
		...
		static void date_missing_parts() {
			var f = TestUtils.write_file("{\"version\" : 1, \"data\": [{\"purpose\": \"foo\", \"amount\": 0, \"date\": {}}]}");
			var passed = false;
			try {
				var builder = ModelBuilderFactory.from_file(f.get_path());
				assert(builder is ModelV1Builder);
		...
		static void date_missing_month() {
			var f = TestUtils.write_file("{\"version\" : 1, \"data\": [{\"purpose\": \"foo\", \"amount\": 0, \"date\": {\"year\": 1234}}]}");
			var passed = false;
			try {
				var builder = ModelBuilderFactory.from_file(f.get_path());
				assert(builder is ModelV1Builder);
		...
		static void date_missing_day() {
			var f = TestUtils.write_file("{\"version\" : 1, \"data\": [{\"purpose\": \"foo\", \"amount\": 0, \"date\": {\"year\": 1234, \"month\": 10}}]}");
			var passed = false;
			try {
				var builder = ModelBuilderFactory.from_file(f.get_path());
				assert(builder is ModelV1Builder);
		...
		static void date_year_invalid_type() {
			var f = TestUtils.write_file("{\"version\" : 1, \"data\": [{\"purpose\": \"foo\", \"amount\": 0, \"date\": {\"year\": [], \"month\": 10}}]}");
			var passed = false;
			try {
				var builder = ModelBuilderFactory.from_file(f.get_path());
				assert(builder is ModelV1Builder);
		...
		static void date_month_invalid_type() {
			var f = TestUtils.write_file("{\"version\" : 1, \"data\": [{\"purpose\": \"foo\", \"amount\": 0, \"date\": {\"year\": 2000, \"month\": []}}]}");
			var passed = false;
			try {
				var builder = ModelBuilderFactory.from_file(f.get_path());
				assert(builder is ModelV1Builder);
		...
		static void date_day_invalid_type() {
			var f = TestUtils.write_file("{\"version\" : 1, \"data\": [{\"purpose\": \"foo\", \"amount\": 0, \"date\": {\"year\": 2000, \"month\": 2, \"date\": []}}]}");
			var passed = false;
			try {
				var builder = ModelBuilderFactory.from_file(f.get_path());
				assert(builder is ModelV1Builder);
